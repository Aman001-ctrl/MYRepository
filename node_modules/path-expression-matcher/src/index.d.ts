 ...
 *
 * Created once by {@link Matcher} and reused across all callbacks â€” no allocation
 ...
 * than at runtime.
 *
 * Obtain via {@link Matcher#readOnly} â€” the same instance is returned every time.
 *
 * @example
 * ```typescript
 ...
 *
 * view.matches(expr);      // âœ“ true
 * view.getCurrentTag();    // âœ“ "user"
 * view.getDepth();         // âœ“ 3
 * // view.push(...)        // âœ— Property 'push' does not exist on type 'MatcherView'
 * ```
 */
 ...
 * user callbacks â€” the same instance is reused on every call with no allocation overhead.
 *
 * @example
 * ```typescript
 * const matcher = new Matcher();
...
export class Matcher {
   ...
   * // same reference every time â€” safe to cache
 ...
 *  - **exact** â€” expressions with a fixed depth and a concrete terminal tag
 *  - **depth-wildcard** â€” fixed depth but terminal tag is `*`
 *  - **deep-wildcard** â€” expressions containing `..` (cannot be depth-indexed)
 *
 * @example
 * ```typescript
...
export class ExpressionSet {
   ...
   *
   * // Later â€” safe: reads are still allowed
   * stopNodes.matchesAny(matcher);
   *
   * // Later â€” throws TypeError: ExpressionSet is sealed
   ...
   *  1. Exact depth + tag â€” O(1) lookup
   *  2. Depth-matched wildcard tag â€” O(1) lookup
   *  3. Deep-wildcard expressions â€” always scanned (typically a small list)
   *
