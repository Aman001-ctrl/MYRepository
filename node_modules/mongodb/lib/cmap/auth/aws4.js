...
async function aws4Sign(options, credentials) {
     ...
     */
    // 1: Create a canonical request
    // Date Ã¢â‚¬â€œ The date and time used to sign the request.
    const date = options.date;
    // RequestDateTime Ã¢â‚¬â€œ The date and time used in the credential scope. This value is the current UTC time in ISO 8601 format (for example, 20130524T000000Z).
    const requestDateTime = date.toISOString().replace(/[:-]|\.\d{3}/g, '');
    // RequestDate Ã¢â‚¬â€œ The date used in the credential scope. This value is the current UTC date in YYYYMMDD format (for example, 20130524).
    const requestDate = requestDateTime.substring(0, 8);
    // Method Ã¢â‚¬â€œ The HTTP request method. For us, this is always 'POST'.
    const method = options.method;
    // CanonicalUri Ã¢â‚¬â€œ The URI-encoded version of the absolute path component URI, starting with the / that follows the domain name and up to the end of the string
    // For our requests, this is always '/'
    const canonicalUri = options.path;
    // CanonicalQueryString Ã¢â‚¬â€œ The URI-encoded query string parameters. For our requests, there are no query string parameters, so this is always an empty string.
    const canonicalQuerystring = '';
    // CanonicalHeaders Ã¢â‚¬â€œ A list of request headers with their values. Individual header name and value pairs are separated by the newline character ("\n").
    ...
    // SignedHeaders Ã¢â‚¬â€œ An alphabetically sorted, semicolon-separated list of lowercase request header names.
    const signedHeaders = canonicalHeaderNames.sort().join(';');
    // HashedPayload Ã¢â‚¬â€œ A string created using the payload in the body of the HTTP request as input to a hash function. This string uses lowercase hexadecimal characters.
    const hashedPayload = await getHexSha256(options.body);
    // CanonicalRequest Ã¢â‚¬â€œ A string that includes the above elements, separated by newline characters.
    ...
    // 2. Create a hash of the canonical request
    // HashedCanonicalRequest Ã¢â‚¬â€œ A string created by using the canonical request as input to a hash function.
    const hashedCanonicalRequest = await getHexSha256(canonicalRequest);
    // 3. Create a string to sign
    // Algorithm Ã¢â‚¬â€œ The algorithm used to create the hash of the canonical request. For SigV4, use AWS4-HMAC-SHA256.
    const algorithm = 'AWS4-HMAC-SHA256';
    // CredentialScope Ã¢â‚¬â€œ The credential scope, which restricts the resulting signature to the specified Region and service.
    ...
    // StringToSign Ã¢â‚¬â€œ A string that includes the above elements, separated by newline characters.
