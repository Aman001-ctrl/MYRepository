...
export async function aws4Sign(
  ...
  // Date â€“ The date and time used to sign the request.
  const date = options.date;
  // RequestDateTime â€“ The date and time used in the credential scope. This value is the current UTC time in ISO 8601 format (for example, 20130524T000000Z).
  const requestDateTime = date.toISOString().replace(/[:-]|\.\d{3}/g, '');
  // RequestDate â€“ The date used in the credential scope. This value is the current UTC date in YYYYMMDD format (for example, 20130524).
  const requestDate = requestDateTime.substring(0, 8);
  // Method â€“ The HTTP request method. For us, this is always 'POST'.
  const method = options.method;
  // CanonicalUri â€“ The URI-encoded version of the absolute path component URI, starting with the / that follows the domain name and up to the end of the string
  // For our requests, this is always '/'
  const canonicalUri = options.path;
  // CanonicalQueryString â€“ The URI-encoded query string parameters. For our requests, there are no query string parameters, so this is always an empty string.
  const canonicalQuerystring = '';

  // CanonicalHeaders â€“ A list of request headers with their values. Individual header name and value pairs are separated by the newline character ("\n").
  ...
  // SignedHeaders â€“ An alphabetically sorted, semicolon-separated list of lowercase request header names.
  const signedHeaders = canonicalHeaderNames.sort().join(';');

  // HashedPayload â€“ A string created using the payload in the body of the HTTP request as input to a hash function. This string uses lowercase hexadecimal characters.
  const hashedPayload = await getHexSha256(options.body);

  // CanonicalRequest â€“ A string that includes the above elements, separated by newline characters.
  ...
  // 2. Create a hash of the canonical request
  // HashedCanonicalRequest â€“ A string created by using the canonical request as input to a hash function.
  const hashedCanonicalRequest = await getHexSha256(canonicalRequest);

  // 3. Create a string to sign
  // Algorithm â€“ The algorithm used to create the hash of the canonical request. For SigV4, use AWS4-HMAC-SHA256.
  const algorithm = 'AWS4-HMAC-SHA256';
  // CredentialScope â€“ The credential scope, which restricts the resulting signature to the specified Region and service.
  ...
  // StringToSign â€“ A string that includes the above elements, separated by newline characters.
