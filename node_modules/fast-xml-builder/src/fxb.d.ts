...
export type XmlBuilderOptions = {
   ...
   * - `isAttribute` â€” `true` when the name being resolved is an attribute name,
   *   `false` when it is a tag name.
   * - `matcher` â€” the current path matcher (readonly). Can be used to inspect the
...
export type SanitizeNameContext = {
   ...
   * Readonly from the callback's perspective â€” do not call mutating methods.
