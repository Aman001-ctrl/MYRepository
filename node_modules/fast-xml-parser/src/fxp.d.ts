 ...
 * 
 * Pattern Syntax:
 * - `root.users.user`      â€” Match exact path
 * - `..user`               â€” Match "user" at any depth (deep wildcard)
 * - `user[id]`             â€” Match user tag with "id" attribute
 * - `user[id=123]`         â€” Match user tag where id="123"
 * - `user:first`           â€” Match first occurrence of user tag
 * - `ns::user`             â€” Match user tag with namespace "ns"
 * - `ns::user[id]:first`   â€” Combine namespace, attribute, and position
 */
 ...
 * callback invocation â€” no allocation overhead per call. Reads directly from
 ...
 * simply absent â€” misuse is caught at compile time by TypeScript.
 *
...
/** Internal node structure â€” exposed via snapshot only. */
