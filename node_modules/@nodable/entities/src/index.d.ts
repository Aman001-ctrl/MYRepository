// ---------------------------------------------------------------------------
// @nodable/entities â€” TypeScript declarations
// ---------------------------------------------------------------------------
...
export interface EntityEncoderOptions {
  ...
  /**
   * Whether to encode nonâ€‘ASCII characters (e.g. `Ã©` â†’ `&eacute;`) using the
   * builtâ€‘in named entity trie.
   * @default true
   */
...
/**
 * Highâ€‘performance encoder that replaces characters with XML/HTML entities.
 *
 * - Escapes XML unsafe characters (`&`, `<`, `>`, `"`, `'`) when `encodeXmlSafe` is true.
 * - Replaces nonâ€‘ASCII characters (e.g. `Ã©`, `Â©`) with named entities using
 *   a compact trieâ€‘based lookup when `encodeAllNamed` is true.
 ...
 * encoder.encode('Â© 2025');  // "&copy; 2025"
 *
 * // With limit
...
export class EntityEncoder {
  ...
  /**
   * Encode a string by replacing XMLâ€‘unsafe characters and (optionally)
   * nonâ€‘ASCII characters with named entities.
   *
 ...
 *
 * - `'external'` â€” only untrusted / injected entities (default)
 * - `'base'`     â€” only builtâ€‘in XML entities + userâ€‘supplied `namedEntities`
 * - `'all'`      â€” all entities regardless of tier
...
export interface EntityDecoderLimitOptions {
   ...
   * - `'base'`               â€“ only builtâ€‘in XML + `namedEntities`
...
export interface EntityDecoderOptions {
   ...
   * These are combined with the builtâ€‘in XML entities (`lt`, `gt`, `quot`, `apos`).
...
/**
 * Singleâ€‘pass, zeroâ€‘regex entity decoder for XML/HTML content.
 *
 * ## Entity lookup priority (highest â†’ lowest)
 * 1. **input / runtime** â€“ injected via `addInputEntities()` (DOCTYPE per document)
 * 2. **persistent external** â€“ set via `setExternalEntities()` / `addExternalEntity()`
 * 3. **base map** â€“ builtâ€‘in XML entities + userâ€‘supplied `namedEntities`
 *
